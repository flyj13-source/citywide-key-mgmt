...
import { FORM_EVENT_LABEL, FORM_COLUMNS, parseScope, isReturnReceipt, type FormEventType } from './keyForm';
...
/**
 * A return receipt attests to a HANDOVER that happened. The date is
 * substituted at render time, because what is being signed is that the keys
 * went back on a specific day — a receipt with no date proves nothing.
 */
const RETURN_ACKNOWLEDGEMENT = (dateText: string): string[] => [
  `I confirm I have returned the keys listed above to City Wide Boston on ${dateText}.`,
  'Any keys shown under "Remaining keys on record" stay in my possession under the same terms:',
  'I will safeguard them, will not duplicate or share them with unauthorized personnel, will',
  'return them upon request or at the end of my assignment, and will report any loss within 24 hours.',
];
...
export async function generateKeyFormPdf(row: any): Promise<string> {
  ...
  // A check-in produces a RETURN RECEIPT: its subject is the keys handed back,
  // not the position left behind. The two documents differ in title, in which
  // table leads, and in what the signature attests to.
  const isReturn = isReturnReceipt(row);
  const returnedLines: any[] = isReturn ? (scope.returned_lines ?? []) : [];

  let y = drawBrandedHeader(
    page, { bold, regular }, logo,
    isReturn ? 'Key Return Receipt' : 'Key Form',
    isReturn
  ...
  page.drawText(isReturn ? 'Return of keys' : `Generated by ${eventLabel}`, {
  ...
  /**
   * One titled table of form lines with its own footing total.
   *
   * `emptyText` is what the section says when it has no rows. A return receipt
   * uses it for the remaining-keys section, where "none" is a real and common
   * answer rather than a defect — the receipt above it still has content.
   */
  const drawSection = (
    opts: { heading?: string | null; lines: any[]; totalLabel: string; emptyText: string; accent?: boolean },
  ) => {
    if (opts.heading) {
      page.drawText(opts.heading.toUpperCase(), { x: 36, y, size: 9, font: bold, color: CW_CHARCOAL });
      y -= 16;
    }
    drawTableHead();
    ...
    if (!opts.lines.length) {
      page.drawText(opts.emptyText, { x: 44, y: y - 2, size: 10, font: regular, color: MUTED });
      y -= 22;
    }

    opts.lines.forEach((line: any, i: number) => {
      if (y < 210) {
        // Continue on a new page rather than writing off the bottom edge.
        page = doc.addPage([612, 792]);
        y = drawBrandedHeader(
          page, { bold, regular }, logo,
          isReturn ? 'Key Return Receipt (continued)' : 'Key Form (continued)',
          row.holder_name,
        );
        drawTableHead();
      }
      ...
      // The role that put them on this row — "AM", "CCM", "IC", "Checked out",
      // or a combination. A number is only checkable if you can see where it
      // came from, and these two sources are tracked independently.
    ...
    page.drawLine({
      start: { x: 36, y: y + 10 }, end: { x: width - 36, y: y + 10 },
      thickness: opts.accent === false ? 0.75 : 1.5,
      color: opts.accent === false ? MUTED : CW_RED,
    });
    y -= 4;
    page.drawText(opts.totalLabel, { x: colX[0], y, size: 10, font: bold, color: CW_CHARCOAL });
    FORM_COLUMNS.forEach((c, ci) => {
      const v = (totals as any)[c.key];
      page.drawText(v ? String(v) : '—', { x: colX[2 + ci] + 6, y, size: 9, font: bold, color: CW_CHARCOAL });
    });
    page.drawText(String(totals.subtotal), {
      x: TOTAL_X + 6, y, size: 12, font: bold,
      color: opts.accent === false ? CW_CHARCOAL : CW_RED,
    });
    y -= 28;
  };

  if (isReturn) {
    // The signable subject leads: these are the keys that came back.
    drawSection({
      heading: 'Keys returned',
      lines: returnedLines,
      totalLabel: 'TOTAL KEYS RETURNED',
      emptyText: 'No keys recorded on this return.',
    });
    // Context underneath. Empty here is a legitimate, common outcome — the
    // person returned everything — and says so plainly.
    if (y < 260) { page = doc.addPage([612, 792]); y = 740; }
    drawSection({
      heading: 'Remaining keys on record',
      lines: scope.lines,
      totalLabel: 'TOTAL STILL HELD',
      emptyText: 'No keys remain on record in this person’s name.',
      accent: false,
    });
  } else {
    drawSection({
      heading: null,
      lines: scope.lines,
      totalLabel: 'TOTAL KEYS HELD',
      emptyText: 'This person currently holds no keys.',
    });
  }
  ...
  const ackLines = isReturn
    ? RETURN_ACKNOWLEDGEMENT(fmt(row.signed_at ?? row.created_at).split(',').slice(0, 2).join(',').trim())
    : ACKNOWLEDGEMENT;
