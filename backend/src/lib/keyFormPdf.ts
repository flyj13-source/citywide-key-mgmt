...
import {
  ...
  DOC_TITLE, DOC_TABLE_HEADING, DOC_TOTAL_LABEL, docKindOf, type FormEventType,
} from './keyForm';
...
export async function generateKeyFormPdf(row: any): Promise<string> {
  ...
  page.drawText(isReturn ? `Return of keys · ${eventLabel}` : `Generated by ${eventLabel}`, {
  ...
  page.drawText(DOC_TABLE_HEADING[kind].toUpperCase(), { x: 36, y, size: 9, font: bold, color: CW_CHARCOAL });
  y -= 16;
  drawTableHead();
  ...
  scope.lines.forEach((line: any, i: number) => {
    if (y < 210) {
      ...
      y = drawBrandedHeader(
        page, { bold, regular }, logo, `${DOC_TITLE[kind]} (continued)`, row.holder_name,
      );
      drawTableHead();
    }
    if (i % 2 === 0) {
  ...
  y -= 4;
  page.drawText(DOC_TOTAL_LABEL[kind], { x: colX[0], y, size: 10, font: bold, color: CW_CHARCOAL });
  FORM_COLUMNS.forEach((c, ci) => {
