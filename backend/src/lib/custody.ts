...
/**
 * The other party to a custody record, when the keys passed directly between
 * two people. The LINKED record is authoritative — a transfer writes both ends
 * — and the explicit counterparty_* columns are the fallback for a handover
 * recorded without a second record of its own.
 */
export function counterpartyOf(row: any): { name: string; email: string | null } | null {
  if (row?.linked_assignment_id) {
    const raw = db.prepare(
      'SELECT assignee, assignee_email FROM key_assignments WHERE id = ?'
    ).get(row.linked_assignment_id) as any;
    if (raw) {
      const other = Object.assign({}, raw);
      if (other.assignee) return { name: other.assignee, email: other.assignee_email ?? null };
    }
  }
  if (row?.counterparty_name) {
    return { name: row.counterparty_name, email: row.counterparty_email ?? null };
  }
  return null;
}
