...
CREATE TABLE IF NOT EXISTS key_assignments (
  ...
  -- Signature DELIVERABILITY, orthogonal to whether a signature exists: it
  -- records whether one could ever be collected. Explicit so "coming" and
  -- "never coming" never look alike:
  ...
  -- Explicit counterparty, for a handover with no linked record of its own.
  -- When linked_assignment_id is set, THAT row is the authoritative other end.
